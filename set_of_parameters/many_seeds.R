...
for(rule in rules) {
  ...
  for(N in 1:20) {
    ...
    for(myseed in 1:max_seed) {
      ...
      output_shape = "outputs/translated_shape/rule/N/N_seed_myseed.png"
      not_a_variable = c("outputs", "translated", "shape", "seed", "png")
...
for(rule in rules) {
  print(rule)
  for(N in seq(from=500, to=5000, by = 500)) {
    print(paste(N, "neighbors"))
    for(myseed in 1:max_seed) {
      print(paste("seed", myseed))
      set.seed(myseed)
      z = simulation_square(N)
      ...
      ## Shape of the output (linked with function shape_output_file)
      output_shape = "outputs/not_translated_shape/rule/N/N_seed_myseed_iteration.png"
      not_a_variable = c("outputs", "not", "translated", "shape", "seed", "png")
      output_folder = shape_output_file(output_shape, only_folder = TRUE,
                                        not_a_variable = not_a_variable)
      folder_create(output_folder)
